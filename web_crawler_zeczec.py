...
def get_crawler_zeczec():
	...
	last_page = int(page_list[-2].get_text()) # 動態頁數的最後一頁，目前最後一頁是11

	output = []
	for page_num in range(1,last_page+1)[0:15]: # 先限制第一頁
		...
		for projects_href in projects_href_list[0:15]:
			projects_url = url + projects_href.get('href')
			result = get_parsing_data(projects_url)
			output.append(result)

	df = pd.DataFrame(output) # DataFrame處理雙維度資料（欄列）
...
def get_parsing_data(projects_url):
	...
	money_text = money[0].get_text().strip() if money else ''  # 空值無法使用int()
	current_money = int(money_text.replace('NT$','').replace(',','')) if money_text else ''

	sponsor_num = int(soup.select('.js-backers-count')[0].get_text().strip()) if len(soup.select('.js-backers-count')) else ''
	
	time_limit = soup.select('.mb-2.text-xs.leading-relaxed')[0].get_text().strip() if len(soup.select('.mb-2.text-xs.leading-relaxed')) > 0 else ''
